...
Run:
    ...
    pip install liteparse fastapi uvicorn python-multipart PyMuPDF
    python server.py
...
def extract_liteparse(pdf_bytes: bytes) -> dict[str, Any]:
    t0 = time.perf_counter()
    # Write to temp file — LiteParse accepts file path or bytes
    with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
        tmp.write(pdf_bytes)
        tmp_path = tmp.name

    try:
        # ocr_enabled=False: only digital text — avoids Tesseract tessdata requirement
        # For scanned docs, text will be empty (reported honestly)
        ...
        return {
            ...
            "bboxes": bboxes[:50],  # first 50 for display
        }
...
def run_parser(text: str, doc_type: str, route: str, filename: str) -> dict[str, Any]:
    if not PARSER_AVAILABLE:
        return {"error": f"Parser not available: {_PARSER_ERR}"}
    if not text.strip():
    ...
    try:
        ...
        fields = result.get("fields", result.get("extracted_data", {}))
...
async def benchmark(
    ...
    for file in files:
        ...
        # Run existing field parser on both texts
        ...
        only_lp = sorted(lp_keys - pm_keys)   # fields LiteParse finds that PyMuPDF misses
        only_pm = sorted(pm_keys - lp_keys)   # fields PyMuPDF finds that LiteParse misses
        both = sorted(pm_keys & lp_keys)

        # Value agreement on shared fields
        disagreements = []
        for k in both:
...
def debug_parser():
    """Test parser in server context."""
    import fitz
    pdf_path = str(Path(__file__).parent.parent / "sample/extracted/Panimalar/Customer Invoice 1ITR2526001878.pdf")
    try:
