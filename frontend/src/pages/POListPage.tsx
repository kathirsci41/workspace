...
export default function POListPage() {
  ...
  if (chainFilter) filters.chain_filter = chainFilter;
  if (missingDocType) filters.missing_doc_type = missingDocType;

  const activeFilterCount = [search, customerFilter, statusFilter, dateFrom, dateTo, chainFilter, missingDocType]
    .filter(Boolean).length + (sortBy !== 'created_at' || sortOrder !== 'desc' ? 1 : 0);

  const resetFilters = () => {
    setSearch(''); setCustomerFilter(''); setStatusFilter('');
    setDateFrom(''); setDateTo(''); setSortBy('created_at');
    setSortOrder('desc'); setChainFilter(''); setMissingDocType('');
    setPage(1);
  };
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-56"
            />
          </div>
          <CustomerCombobox
            customers={customers?.items ?? []}
            value={customerFilter}
            onChange={(id) => { setCustomerFilter(id); setPage(1); }}
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setSortBy(col); setSortOrder(ord); setPage(1);
            }}
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="created_at:desc">Newest First</option>
            <option value="created_at:asc">Oldest First</option>
            <option value="po_date:desc">PO Date ↓</option>
            <option value="po_date:asc">PO Date ↑</option>
            <option value="chain_completeness:desc">Most Complete</option>
            <option value="chain_completeness:asc">Least Complete</option>
            <option value="total_amount:desc">Amount ↓</option>
          </select>
        </div>

        {/* Filters — Row 3: chain completeness pills + missing doc type */}
        <div className="flex flex-wrap gap-3 items-center">
          <div className="flex gap-1">
            {[
              ...
              <button
                key={opt.value}
                onClick={() => { setChainFilter(opt.value); setPage(1); }}
                className={clsx(
          ...
          <select
            ...
            onChange={(e) => { setMissingDocType(e.target.value); setPage(1); }}
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="">Any Documents</option>
            {DOC_TYPE_OPTIONS.map((opt) => (
              <option key={opt.value} value={opt.value}>Missing {opt.label}</option>
            ))}
          </select>
          ...
          {activeFilterCount > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-red-600 border border-gray-300 hover:border-red-300 px-3 py-1.5 rounded-full"
            >
              <X size={12} />
