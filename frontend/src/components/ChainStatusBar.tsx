...
function pickMessage(slots: ChainSlot[]): string | null {
  // Show message from the non-verified, most recent problematic slot
  const problem = slots.find(
    (s) => s.status !== 'VERIFIED' && s.slot_message
  );
  return problem?.slot_message ?? null;
}
                ...
                <div className="relative group">
                  ...
                  {message && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-20 w-56 pointer-events-none">
                      <div className="bg-gray-900 text-white text-xs rounded-lg px-3 py-2 leading-snug shadow-lg">
                        {message}
                      </div>
